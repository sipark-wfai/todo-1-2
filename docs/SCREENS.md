...
| ⑫ | 완료 체크박스 | submit (`?/toggle`) | 즉시 토글. 필터·정렬 조건 유지 | FR-107 |
...
| ⑰ | 삭제 | submit (`?/delete`) | 확인 단계 후 실행 | FR-108 |
...
- ⑫·⑰은 각각 독립 `<form method="POST">`이며, 현재 쿼리 파라미터를 hidden 필드로 넘겨 조작 후 조건이 유지된다 (US-104 수용 기준 3).
- JS가 있으면 `use:enhance`로 전체 리로드 없이 처리한다 (NFR-401).
- ⑰ 삭제 확인: JS 있으면 확인 다이얼로그, 없으면 두 단계 제출(확인 화면 경유).
...
| No | 명칭 | 타입 | 동작 / 검증 | 관련 FR |
|---|---|---|---|---|
| ① | 목록으로 | link → `/` | 진입 시 쿼리 파라미터를 보존해 복귀 | — |
...
| ⑧ | 완료 | checkbox | 켜면 완료 시각 기록, 끄면 제거 | FR-107 |
| ⑨ | 저장 | submit (`?/update`) | 성공 시 `/`로 리다이렉트, `updated_at` 갱신 | FR-105, FR-106 |
| ⑩ | 취소 | link → `/` | 변경 사항 폐기 | — |
| ⑪ | 삭제 | submit (`?/delete`) | 확인 후 실행, `/`로 이동 | FR-108, FR-109 |
...
| ⑥ | 삭제 | submit (`?/delete`) | 확인 문구에 연결 건수 명시. 할 일은 삭제되지 않음 | FR-404 |
...
3. 목록 최상단에 새 항목이 나타나고 ①이 비워지며 포커스 유지
...
### SN-03. 할 일 정리 (속성 부여)
1. SC-01에서 ⑬ 제목 클릭 → SC-02
2. ④ 마감일, ⑤ 우선순위, ⑥ 태그 설정
3. ⑨ 저장 → `/`로 복귀, `updated_at` 갱신

### SN-04. 완료 처리
1. SC-01에서 ⑫ 체크박스 클릭 → `POST /?/toggle`
2. 완료 시각 기록, 항목이 완료 스타일로 전환
3. 현재 필터·검색·정렬 조건 유지 (hidden 필드로 전달)

### SN-05. 태그 삭제 (부작용 확인)
1. SC-03에서 ⑤ 연결 건수 확인
2. ⑥ 삭제 → 확인 단계에 연결 건수 표시
3. 확인 → 태그와 연결만 제거. **할 일은 유지** (FR-404)

### SN-06. JS 비활성 환경 (NFR-401, NFR-402)
1. 브라우저 JS 비활성 상태로 `/` 진입
2. ① 입력 → ② 추가: 페이지 리로드 후 항목 추가됨
3. ⑫ 체크박스: 폼 제출 → 리로드 후 완료 상태 반영
4. ④~⑨ 필터: GET 폼 제출로 동작
5. ⑰ 삭제: 확인 화면 경유 후 삭제
...
| 폭 | 필터 바 | 항목 행 |
|---|---|---|
| ~ 640px | select 세로 스택 | 체크박스 + 제목 / 메타 정보 2줄 |
| 641px ~ | 한 줄 배치 | 좌: 체크박스·제목, 우: 메타·삭제 |
